...
function GameCard({ game, featured = false }: { game: CatalogGame; featured?: boolean }) {
  ...
  const classes = `group flex min-h-32 h-full items-center gap-4 rounded-lg border border-border bg-surface/80 p-5 text-left transition-colors hover:border-danger/70 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-danger ${featured ? "border-danger/50 bg-elevated" : ""}`;
