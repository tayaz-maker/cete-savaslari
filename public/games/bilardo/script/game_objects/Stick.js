...
Stick.prototype.handleInput = function (delta) {
    ...
    if (typeof window.__poolPower === "number") {
      var target = Math.max(0, Math.min(75, window.__poolPower));
      this.power = target;
      this.origin.x = 970 + (target / 1.2) * 2;
    }
    ...
    else if ((this.power>0 && Mouse.left.down) || (this.power>0 && window.__poolShoot)){
      window.__poolShoot = false;
