...
function bindTouchAsMouse() {
    function synth(touch) {
        return { pageX: touch.pageX, pageY: touch.pageY, which: 1 };
    }
    var opts = { passive: false };
    document.addEventListener("touchstart", function (e) {
        if (e.target && e.target.closest && e.target.closest("#touchHud")) return;
        if (e.changedTouches[0]) {
            handleMouseDown(synth(e.changedTouches[0]));
            e.preventDefault();
        }
    }, opts);
    document.addEventListener("touchmove", function (e) {
        if (e.target && e.target.closest && e.target.closest("#touchHud")) return;
        if (e.changedTouches[0]) {
            handleMouseMove(synth(e.changedTouches[0]));
            e.preventDefault();
        }
    }, opts);
    document.addEventListener("touchend", function (e) {
        if (e.target && e.target.closest && e.target.closest("#touchHud")) return;
        if (e.changedTouches[0]) handleMouseUp(synth(e.changedTouches[0]));
    }, opts);
}
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", bindTouchAsMouse);
} else {
    bindTouchAsMouse();
}
