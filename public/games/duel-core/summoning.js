...
export function specialPlans(state,player,credit=0) {
  ...
  for(const uid of p.auxiliary) {
    ...
    const available=[...new Set((req.zones||['hand','units']).flatMap(zone=>p[zone]))].filter(id=>id&&definition(state,id).kind==='unit');
...
export function ritualPlans(state,player) {
  const p=state.players[player],plans=[];
  if(p.flags.blockSpecial?.until>=state.turn)return plans;
  ...
  for(const uid of p.hand) {
    ...
    const available=[...p.hand,...p.units].filter(id=>id&&id!==uid&&definition(state,id).kind==='unit');
    ...
    for(let count=1;count<=available.length;count++) {
      ...
      for(const material of sets)for(const extra of [null,...grave]) {
        ...
        if(level>=requirement&&(p.units.includes(null)||materials.some(id=>p.units.includes(id))))plans.push({card:uid,materials,ritual:true});
      }
