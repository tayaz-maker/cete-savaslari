...
export function deserialize(raw, pool, theme) {
  try {
    ...
    if (envelope.version !== 1 || checksum(envelope.payload) !== envelope.checksum) return { ok: false, error: 'corrupt-save' };
    const state = { ...JSON.parse(envelope.payload), catalog: Object.fromEntries(pool.map(c => [c.id, c])) };
    if (state.theme !== theme || !validateState(state)) return { ok: false, error: 'invalid-save' };
    return { ok: true, state };
  } catch { return { ok: false, error: 'corrupt-save' }; }
}
export function saveDuel(storage, state) {
  try {
    const key = saveKey(state.theme), raw = serialize(state), previous = storage.getItem(key);
    if (previous && deserialize(previous, Object.values(state.catalog), state.theme).ok) storage.setItem(`${key}.backup`, previous);
