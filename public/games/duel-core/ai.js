...
export function chooseAction(view, actions) {
  ...
  for (const action of actions) {
    const card = view.cards[action.card], target = view.cards[action.target];
    let value = -1000;
    ...
    if (action.type === 'summon') value = 100 + card.attack / 100 - (action.tributes || []).reduce((sum, uid) => sum + (view.cards[uid]?.attack || 0) / 100, 0);
    if (action.type === 'special') value = 105 + (card?.attack||0)/100 - (action.materials||[]).reduce((sum,uid)=>sum+(view.cards[uid]?.attack||0)/100,0);
    if (action.type === 'set-unit') value = 95 + card.defense / 100 - (action.tributes || []).length * 20;
    if (action.type === 'set-support') value = card.kind === 'trap' || card.subtype === 'quick' ? 60 : -20;
    if (action.type === 'activate') value = 80;
    if (action.type === 'position') value = card.face === 'down' || (card.position === 'defense' && card.attack > card.defense) ? 25 : -20;
    if (action.type === 'attack') {
      if (!action.target) value = card.attack >= opponent.points ? 10000 : 200 + card.attack / 100;
      else {
        const enemy = target?.face === 'down' ? 1200 : target?.position === 'defense' ? target.defense : target.attack;
