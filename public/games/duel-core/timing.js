...
export function flagActive(state,player,name) {const f=state.players[player].flags[name];return f&&f.until>=state.turn?f.value:false;}
export function hasTrait(state,player,name) {return standing(state,player).some(uid=>definition(state,uid).traits?.[name]);}
export function canRespond(state,uid,player) {
  ...
  const delay=standing(state,1-player).reduce((sum,id)=>sum+(definition(state,id).traits?.trapDelay||0),0);
  if(d.kind==='trap') {
    if(!set||flagActive(state,player,'blockTrap'))return false;
    const early=c.setTurn===state.turn&&hasTrait(state,player,'sameTurnTrapOnce')&&p.used.sameTurnTrap!==state.turn;
    if(!early&&state.turn-c.setTurn<1+delay)return false;
    if(a.type==='attack'&&state.cards[a.target]?.position==='defense'&&definition(state,a.target)?.traits?.blockTrapWhenDefending)return false;
  }
  if(d.kind==='spell') {
    ...
    if(hand&&(hasTrait(state,0,'quickMustSet')||hasTrait(state,1,'quickMustSet')))return false;
  }
  ...
  if(t.responseDrawEffect&&!source?.effects.some(op=>['draw','drawSetTrap'].includes(op.op)))return false;
  if(t.responseDestroy&&!['attack','destroy'].includes(a.type)&&!source?.effects.some(op=>op.op==='destroy'))return false;
  if(t.responseTargetOrBattle&&a.type!=='attack'&&!source?.effects.some(op=>op.op==='select'))return false;
  if(source?.kind==='trap'&&hasTrait(state,a.player,'trapUnnegatable')&&d.effects.some(op=>op.op==='negate'))return false;
  if(t.destroyedSeries&&(!source?.series.includes(t.destroyedSeries)||a.type!=='destroy'))return false;
  return true;
}
