...
export function matches(state, uid, filter = {}, player = 0) {
  const card = state.cards[uid], def = definition(state, uid), at = locate(state, uid);
  ...
  if (filter.bossOrLevelOne && def.level !== 1 && ![].concat(def.series).includes('Baba')) return false;
  if (filter.sameLevel && filter.reference && def.level !== definition(state,filter.reference)?.level) return false;
  if (['units','support','field'].includes(at.zone) && filter.effect !== false && !effectTargetable(state,uid,player)) return false;
  return true;
}
export function candidates(state, player, selector = {}) {
  const owners = selector.owner === 'opponent' ? [1-player] : selector.owner === 'both' ? [0,1] : [player];
  const zones = [].concat(selector.zones || 'units');
  return owners.flatMap(who => zones.flatMap(zone => zone === 'field' ? [state.players[who].field].filter(Boolean) : state.players[who][zone] || []))
...
export function trait(state, uid, key) {
  ...
  if (!card || !def || !faceUp(state, uid) || card.negated || card.negatedUntil >= state.turn) return null;
