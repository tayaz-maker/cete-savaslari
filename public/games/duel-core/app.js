...
const $=(tag,attrs={},...children)=>{
 ...
 for(const [key,value]of Object.entries(attrs)) {
  ...
  else if(value!==false&&value!==null&&value!==undefined)el.setAttribute(key,value===true?'':String(value));
 }
...
export async function startApp(theme,designs) {
 ...
 const button=(label,fn,attrs={})=>$('button',{type:'button',onclick:fn,...attrs},label);
 ...
 const displayError=error=>error==='storage-failed'?t('saveError'):error==='no-save'?t('noSave'):t('corrupt');
 ...
 function show(title,body,cls='') {
  if(dialog.open)dialog.close();dialog.className=cls;
  dialog.replaceChildren($('div',{class:'dialog-head'},$('h2',{id:'dialog-title'},title),button(t('close'),close,{'aria-label':t('close')})), $('div',{class:'dialog-body'},body));
  dialog.showModal();
 }
 dialog.addEventListener('click',e=>{if(e.target===dialog)close();});
 document.addEventListener('keydown',e=>{if(e.key==='Escape'){selected=null;if(!dialog.open&&screen==='duel')render();}});
 function ask(message,yes){show(name,[$('p',{},message),$('div',{class:'dialog-actions'},button(t('cancel'),close),button(t('confirm'),()=>{close();yes();},{class:'primary'}))]);}
 ...
 function menu(){return $('main',{class:'menu-stage'},$('section',{class:'front'},$('img',{class:'emblem',src:`/games/${theme}/assets/emblem.svg`,alt:''}),$('div',{class:'eyebrow'},theme==='veto-h'?(lang==='tr'?'SEÇİM GECESİ · KART DÜELLOSU':'ELECTION NIGHT · CARD DUEL'):(lang==='tr'?'İSTANBUL GECESİ · KART DÜELLOSU':'ISTANBUL NIGHT · CARD DUEL')),$('h1',{},name),$('p',{},t('deckNote')),$('div',{class:'menu-buttons'},button(t('new'),()=>{if(saved?.ok||saved?.error!=='no-save')ask(t('overwrite'),beginSetup);else beginSetup();},{class:'primary'}),button(t('continue'),()=>{if(saved?.ok){state=saved.state;screen='duel';render();scheduleAI();}else{notice=displayError(saved?.error);render();}},{disabled:!saved?.ok}),button(`${t('archive')} · ${pool.length}`,()=>{screen='archive';render();}),button(t('help'),()=>show(t('help'),$('p',{},t('rules')))),button(t('settings'),settings),$('a',{href:'/'},`← ${t('back')}`))));}
 function settings(){show(t('settings'),[$('label',{},t('motion'),' ', $('select',{onchange:e=>{motion=e.target.value;try{localStorage.setItem('tariklab.duel.motion',motion);}catch{/* Preference remains active for this visit. */}document.body.dataset.motion=motion==='on'?'full':'reduced';}},$('option',{value:'on',selected:motion==='on'},t('motionOn')),$('option',{value:'reduced',selected:motion!=='on'},t('motionOff'))))]);}
 function beginSetup(){const seed=crypto.getRandomValues(new Uint32Array(1))[0];setup={seed,rng:seed,first:null};rps();}
 function rps(message=''){show(t('rps'),[$('p',{},message||t('deckNote')),$('div',{class:'dialog-actions'},...['rock','paper','scissors'].map((key,i)=>button(t(key),()=>{
  const enemy=Math.floor(random(setup)*3),difference=(i-enemy+3)%3;
  if(!difference){rps(`${t(key)} / ${t(['rock','paper','scissors'][enemy])}. ${t('rpsTie')}`);return;}
  if(difference===1)show(t('rps'),[$('p',{},t('rpsWin')),$('div',{class:'dialog-actions'},button(t('first'),()=>prepare(0)),button(t('second'),()=>prepare(1)))]);
  else prepare(1,t('rpsLose'));
 })))]);}
 function prepare(first,message=''){setup.first=first;setup.decks=[0,1].map(p=>generateDeck(pool,(setup.seed+Math.imul(p+1,2654435761))>>>0));show(t('new'),[$('p',{},message||t(first===0?'first':'second')),$('p',{},t('deckNote')),button(t('preview'),()=>show(t('preview'),[$('p',{},t('previewNote')),...setup.decks[0].main.map(id=>pool.find(c=>c.id===id)).sort((a,b)=>text(a.name).localeCompare(text(b.name),lang)).map(c=>$('div',{},text(c.name))),button(t('back'),()=>prepare(first,message))])),button(t('start'),()=>{state=createDuel(pool,theme,setup.seed,setup.first,setup.decks);setup=null;screen='duel';lastPoints=null;close();save();render();scheduleAI();},{class:'primary'})]);}
 ...
 function optionName(id){const option=state?.choice?.options?.find(o=>o.id===id);return option?.card?cname(option.card):({pay:lang==='tr'?'Puan öde':'Pay points',keep:lang==='tr'?'Tut':'Keep',destroy:lang==='tr'?'Yok et':'Destroy',discard:t('discard'),tribute:t('tributes'),set:t('set-support')})[id]||t('choose');}
 function selectAction(list){
  if(list.length===1){confirmAction(list[0]);return;}
  let page=0;
  function choices(){show(t('action'),[$('div',{class:'choice-list'},...list.slice(page*12,page*12+12).map(a=>button(actionTitle(a),()=>confirmAction(a)))),$('div',{class:'dialog-actions'},button(t('previous'),()=>{page--;choices();},{disabled:page===0}),button(t('next'),()=>{page++;choices();},{disabled:(page+1)*12>=list.length}))]);}
  choices();
 }
 function confirmAction(action){show(t('confirm'),[$('p',{},action.card?cname(action.card):t('choice')),$('p',{},actionTitle(action)),$('div',{class:'dialog-actions'},button(t('cancel'),close),button(t('confirm'),()=>{close();command(action);},{class:'primary'}))]);}
 function inspectBody(uid,v=view()){
  ...
  if(card.name&&card.owner===0)for(const type of ['summon','activate','position','attack'])if(!groups.includes(type)) {
   ...
   body.push($('div',{},button(t(type),()=>{},{disabled:true}),$('small',{},` ${reason(error)}`)));
  }
  return body;
 }
 function inspect(uid){show(t('inspector'),inspectBody(uid),'inspector-sheet');}
 function pile(player,key,v){
  if(key==='deck')return;const ids=key==='auxiliary'?Object.values(v.cards).filter(c=>c.owner===player&&c.deckLocation==='auxiliary').map(c=>c.uid):key==='field'?[v.players[player].field].filter(Boolean):v.players[player][key];
  show(t(key),ids?.length?ids.map(uid=>button(cname(uid,v),()=>inspect(uid))):$('p',{},t('empty')));
 }
 function playerField(player,v){
  ...
  return $('section',{class:`player-field ${isPlayer?'player':'opponent'}`,'aria-label':t(isPlayer?'you':'opponent')},$('div',{class:'player-meter'},$('span',{},t(isPlayer?'you':'opponent'),` · ${t('hand')} ${p.handCount}`),$('strong',{class:changed?'damage':''},`${p.points.toLocaleString(lang)} ${point}`)),...(isPlayer?rows.slice().reverse():rows),$('div',{class:'piles'},$('span',{},`${t('deck')} ${p.deckCount}`),...['auxiliary','grave','banished','field'].map(key=>button(`${t(key)} ${key==='auxiliary'?p.auxiliaryCount:key==='field'?(p.field?1:0):p[key].length}`,()=>pile(player,key,v),{disabled:key==='auxiliary'&&player===1}))));
 }
 function logLine(e,v){
  const who=t(e.player===0?'you':'opponent');
  if(e.event==='battle')return `${who}: ${t('battle')} · ${cname(e.attacker,v)} · ${e.damage.join(' / ')} ${point}`;
  if(e.event==='draw')return `${who}: ${t('draw')} +${e.count}`;
  if(e.event==='move')return `${who}: ${cname(e.uid,v)} → ${t(e.to==='hand'?'hand':e.to==='units'?'unit':e.to)}`;
 ...
 function board(){
  const v=view(),approved=actions(),phase=approved.find(a=>a.type==='phase'),pass=approved.find(a=>a.type==='pass');
  const logBody=$('ol',{},...v.log.slice(-20).reverse().map(e=>$('li',{},$('small',{},`${t('turn')} ${e.turn} · `),logLine(e,v))));
  const status=v.result?t(v.result.winner===null?'tie':v.result.winner===0?'win':'lose'):actor()===1?t('thinking'):v.choice?t('choice'):v.pending?t('response'):t('yourMove');
 ...
 function archive(){
  ...
  const select=(key,value,options,onChange)=>$('select',{'aria-label':t(key),onchange:e=>{onChange(e.target.value);archivePage=0;render();}},$('option',{value:''},`${t(key)} · ${t('all')}`),...options.map(([id,label])=>$('option',{value:id,selected:id===value},label)));
  return $('main',{class:'archive'},$('div',{class:'archive-head'},$('h1',{},t('archive')),$('span',{},`${filtered.length} / ${pool.length}`)),$('div',{class:'filters'},$('input',{type:'search',placeholder:t('search'),'aria-label':t('search'),value:archiveQuery,oninput:e=>{archiveQuery=e.target.value;archivePage=0;const cursor=e.target.selectionStart;render();const input=root.querySelector('input');input.focus();input.setSelectionRange(cursor,cursor);}}),select('all',archiveKind,['unit','spell','trap'].map(k=>[k,t(k)]),v=>archiveKind=v),select('series',archiveSeries,[...new Set(pool.flatMap(c=>c.series))].map(s=>[s,s]),v=>archiveSeries=v),select('level',archiveLevel,[...new Set(pool.filter(c=>c.kind==='unit').map(c=>c.level))].sort((a,b)=>a-b).map(n=>[String(n),String(n)]),v=>archiveLevel=v)),$('div',{class:'archive-grid'},...filtered.slice(archivePage*perPage,(archivePage+1)*perPage).map(c=>cardEl(c,c.id,()=>archiveInspect(c)))),$('nav',{class:'pagination'},button(t('previous'),()=>{archivePage--;render();},{disabled:archivePage===0}),$('span',{},`${t('page')} ${archivePage+1} / ${Math.max(1,Math.ceil(filtered.length/perPage))}`),button(t('next'),()=>{archivePage++;render();},{disabled:(archivePage+1)*perPage>=filtered.length})));
 }
 try {
  const response=await fetch(`/games/${theme}/source-cards.json`);if(!response.ok)throw Error(String(response.status));
  pool=buildCards(await response.json(),designs,theme);saved=loadDuel(localStorage,pool,theme);
  if(saved.recovered)notice=t('recovered');else if(!saved.ok&&saved.error!=='no-save')notice=displayError(saved.error);
  render();
 }catch(error){root.replaceChildren($('main',{class:'menu-stage'},$('section',{class:'front'},$('h1',{},name),$('p',{},t('loadError')),button(t('retry'),()=>location.reload()),$('a',{href:'/'},t('back')))));console.error(error);}
