...
export function battlePacket(state, attacker, defender) {
  ...
  if (!defender) return { damage: [0, Math.floor(attack * (traits(state,attacker).fullDirect || modified(state,attacker,'fullDirect') ? 1 : trait(state, attacker, 'directMultiplier') ?? 1))], destroy: [] };
  const defense = state.cards[defender].position === 'defense' && !trait(state,attacker,'attackDefenseAsAttack') && !modified(state,defender,'defendWithAttack');
  const value = stat(state, defender, defense ? 'defense' : 'attack');
  if (defense) return { damage: [Math.max(0, value - attack), 0], destroy: attack > value ? [defender] : [] };
  const packet = {
...
export function resolveBattle(ctx, action) {
  const state = ctx.state, attacker = action.card, defender = action.target;
  if (!state.players[action.player].units.includes(attacker) ||
      (defender && !state.players[1 - action.player].units.includes(defender))) {
    log(state, 'attack-target-left'); return;
  }
  if (defender && state.cards[defender].face === 'down') {
    ...
    if (state.work.length) { ctx.deferBattle(action); return; }
    if (!state.players[action.player].units.includes(attacker) || !state.players[1 - action.player].units.includes(defender)) return;
  }
  if(!defender)state.players[action.player].points-=traits(state,attacker).directCost||0;
  const packet = battleDamage(state,attacker,defender,battlePacket(state, attacker, defender),action.player);
  ...
  for (const uid of packet.destroy) {
    const value=stat(state,uid,'attack'),wasSet=state.cards[uid]?.battleWasSet;
    const destroyed=ctx.destroy(uid, 'battle',uid===defender&&traits(state,attacker).battleBanish);
