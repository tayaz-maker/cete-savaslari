...
function combinations(values, count) {
  ...
  return values.flatMap((value, i) =>
    combinations(values.slice(i + 1), count - 1).map((rest) => [value, ...rest]),
  );
}
export function legalActions(state, player) {
  ...
  if (state.choice)
    return state.choice.player !== player
      ? []
      : state.choice.kind === "option"
        ? state.choice.options.map((o) => command({ type: "choose", option: o.id }))
        : combinations(state.choice.ids, state.choice.count).map((targets) =>
            command({ type: "choose", targets }),
          );
  if (state.pending)
    return state.pending.responding !== player
      ? []
      : [
          command({ type: "pass" }),
          ...responseCards(state).map((card) => command({ type: "respond", card })),
        ];
  ...
  for (const card of p.hand) {
    ...
    if (def.kind === "unit") {
      ...
      const costs = [
        ...combinations(p.units.filter(Boolean), n),
        ...(alternate
          ? combinations(
              p[alternate.zone].filter((uid) => definition(state, uid).kind === alternate.kind),
              alternate.count,
            )
          : []),
      ];
      for (const tributes of costs)
        for (let slot = 0; slot < 5; slot++) {
          candidates.push(
            command({ type: "summon", card, slot, tributes }),
            command({ type: "set-unit", card, slot, tributes }),
          );
        }
    } else
      for (let slot = 0; slot < 5; slot++)
  ...
  const expanded = candidates.flatMap((action) => {
    ...
    for (const group of targetOptions(state, player, action.card)) {
      variants = variants.flatMap((a) =>
        combinations(group.ids, group.count || 1).map((ids) => ({
          ...a,
          targets: [...a.targets, ...ids],
        })),
      );
    }
    return variants;
