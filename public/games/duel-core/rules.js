...
import { PHASES, MAIN, ROWS, definition, locate, activeCards, log, finish, checkPoints, validateState, canonicalize } from './model.js';
...
export function createDuel(pool, theme, seed, first = 0, prepared = null) {
  const state = { version: 1, theme, seed: seed >>> 0, rng: seed >>> 0, revision: 0,
    first, active: first, turn: 1, phase: 'draw', cards: {}, catalog: Object.fromEntries(pool.map(c => [c.id, c])),
    players: [], log: [], pending: null, result: null, work: [], choice: null, effectSerial: 0 };
  for (let player = 0; player < 2; player++) {
    const deck = prepared?.[player] || generateDeck(pool, (seed + Math.imul(player + 1, 2654435761)) >>> 0);
    const p = { points: 8000, deck: [], auxiliary: [], hand: [], grave: [], banished: [], units: Array(5).fill(null),
      support: Array(5).fill(null), field: null, normalUsed: 0, flags: {}, used: {}, fieldHistory: [] };
    for (const zone of ['deck', 'auxiliary']) {
      const ids = zone === 'deck' ? deck.main : deck.auxiliary;
      ids.forEach((id, index) => {
        const uid = `${player}:${zone}:${index}`;
        state.cards[uid] = { id, owner: player, face: 'down', position: 'defense', setTurn: 0, summonedTurn: 0,
          positionTurn: 0, attacksUsed: 0, modifiers: [], used: {}, knownTo: [player === 0, player === 1] };
        p[zone].push(uid);
      });
    }
...
function context(state, player, source = null, targets = []) {
  ...
  ctx.special = plan => {
    ...
    const slot=state.players[player].units.indexOf(null);if(slot<0)throw Error('Special summon requires a zone');
    ctx.move(plan.card,'units','special',player,slot);
    const c=state.cards[plan.card];c.face='up';c.position='attack';c.summonedTurn=state.turn;c.knownTo=[true,true];
    if(plan.reveal) {state.cards[plan.reveal].knownTo=[true,true];state.players[1-player].flags.lockedName={value:definition(state,plan.reveal).name.tr,until:state.turn};}
  ...
  ctx.token = op => {
    for (let i = 0; i < (op.count || 1); i++) {
      ...
      state.cards[uid] = {id:'token',owner:player,face:'up',position:op.position||'defense',
        token:{tr:op.tr||'Temsilci',en:op.en||'Representative',attack:op.attack||0,defense:op.defense||0},
        setTurn:state.turn,summonedTurn:state.turn,positionTurn:state.turn,attacksUsed:0,modifiers:[],used:{},knownTo:[true,true]};
  ...
  ctx.move = (uid, to, reason = 'effect', destination = state.cards[uid]?.owner, slot = null) => {
    ...
    if(['deck','hand'].includes(to)&&definition(state,uid).deckLocation==='auxiliary')to='auxiliary';
    if(where.zone==='grave'&&reason!=='material'&&hasTrait(state,1-where.player,'lockEnemyGrave'))return false;
    if (ROWS.includes(to) && (slot === null || state.players[destination][to][slot] !== null)) return false;
    ...
    if (state.cards[uid].token && !ROWS.includes(to)) {
      delete state.cards[uid]; log(state,'token-left',{player:where.player,uid,reason}); return true;
    }
    ...
    if (['units','support','field'].includes(where.zone)&&!['units','support','field'].includes(to)) {
      state.cards[uid].modifiers=[];state.cards[uid].negated=false;delete state.cards[uid].negatedUntil;
      for(const owner of [0,1])for(const equip of [...state.players[owner].support]) {
        if(equip&&state.cards[equip].equippedTo===uid)ctx.move(equip,'grave','equip-target-left');
      }
    }
    if (['grave', 'banished'].includes(to)) {
      if(to==='banished')state.cards[uid].banishReason=reason;
      state.cards[uid].face = 'up'; state.cards[uid].knownTo = [true, true];
      state.cards[uid].modifiers = []; state.cards[uid].negated = false;
    }
  ...
  ctx.destroy = (uid, reason, banish = false) => {
    ...
    const traits = suppressed(state,uid)?{}:definition(state, uid).traits || {}, card = state.cards[uid];
    const protection = reason === 'battle' ? traits.battleProtection : traits.effectProtection;
    if (protection === 'always') return false;
    if (protection === 'turn' && card.used.protection !== state.turn) { card.used.protection = state.turn; return false; }
    if (protection === 'duel' && !card.used.duelProtection) { card.used.duelProtection = true; return false; }
    if (card.modifiers.some(m => (m.until === null || m.until >= state.turn) && (m.protectAll || m[reason === 'battle' ? 'protectBattle' : 'protectEffect']))) return false;
    const owner = locate(state, uid).player;
    const t=suppressed(state,uid)?{}:traits;
    if ((t.fieldProtection || reason==='battle'&&t.fieldBattleProtection) && state.players.some(p=>p.field)) return false;
    if(reason==='battle'&&t.fieldBattleOnce&&state.players.some(p=>p.field&&definition(state,p.field).id===t.fieldBattleOnce)&&!card.used.fieldProtection){card.used.fieldProtection=true;return false;}
    for(const source of standing(state,owner)) {
      const sourceTraits=definition(state,source).traits||{},sourceCard=state.cards[source];
      if(reason==='battle'&&sourceTraits.seriesBattleProtection&&hasSeries(state,uid,sourceTraits.seriesBattleProtection))return false;
      if(reason==='effect'&&sourceTraits.protectOwnSetUnits&&card.face==='down'&&definition(state,uid).kind==='unit')return false;
      if(card.face==='down'&&(sourceTraits.protectSetOnce||reason==='effect'&&sourceTraits.protectSetEffectOnce)&&sourceCard.used.setProtection!==state.turn){sourceCard.used.setProtection=state.turn;return false;}
      if(reason==='battle'&&sourceCard.equippedTo===uid&&sourceTraits.equip?.battleDestroyedDraw)runEffects({...ctx,player:owner,source,targets:[]},[{op:'draw',count:sourceTraits.equip.battleDestroyedDraw}]);
    }
  ...
  ctx.emit = (event, data) => {
    ...
    for (const uid of listeners) {
      ...
      if (event === 'draw' && owner !== data.player && t.revealEnemyDrawSpell && state.cards[uid].used.drawReveal !== state.turn) {
        ...
        if (definition(state,data.uid).kind === 'spell') ctx.move(data.uid,'grave','revealed-draw');
      }
      for (const effect of def?.triggers || []) {
        if (effect.event !== event || (!effect.global && uid !== data.uid) ||
            (effect.own && owner !== data.player) || (effect.opponent && owner === data.player) ||
            (effect.normal && !data.normal) || (effect.tribute && !data.tributes?.length) ||
            (effect.from && effect.from !== data.from) ||
            (effect.tributeSeries && !data.tributes?.some(id => [].concat(effect.tributeSeries).some(series => hasSeries(state,id,series)))) ||
            (effect.sameLevel && definition(state,data.target)?.level !== def.level) ||
            (effect.requiresSeries && !state.players[owner].units.some(id => id && hasSeries(state,id,effect.requiresSeries))) ||
            (effect.reason && effect.reason !== data.reason)) continue;
...
function settle(state) {
  ...
  while (state.work.length && !state.choice && !state.result) {
    ...
    if (job.type === 'resolve') { resolve(state, job.action); continue; }
    ...
    if (job.type === 'battle') { resolveBattle(ctx, job.action); continue; }
...
export function rejection(state, action) {
  ...
  if (state.choice) {
    ...
    if (c.kind === 'option') return c.options.some(o=>o.id===action.option)?null:'legal-option-required';
    const ids = action.targets || [];
    return ids.length === c.count && new Set(ids).size === ids.length && ids.every(uid => c.ids.includes(uid) && locate(state, uid)) ? null : 'legal-target-required';
  }
  if (state.pending) {
  ...
  const p = state.players[action.player], card = state.cards[action.card], def = definition(state, action.card);
  if (action.type === 'phase') {
    if (state.phase === 'end' && p.hand.length > 6) return 'hand-limit';
    return null;
  }
  if (action.type === 'discard') return state.phase === 'end' && p.hand.length > 6 && p.hand.includes(action.card) ? null : 'hand-limit-only';
  ...
  if (['summon', 'set-unit'].includes(action.type)) {
    if (!MAIN.includes(state.phase)) return 'main-phase-only';
    if (!p.hand.includes(action.card) || def?.kind !== 'unit' || def.subtype === 'ritual') return 'invalid-unit';
    if (state.phase === 'main1' && state.players[1-action.player].units.some(uid=>uid && state.cards[uid].face==='down' && definition(state,uid).traits?.facedownBlockNormalMain1)) return 'normal-blocked';
    const extra=standing(state,action.player).find(uid=>definition(state,uid).traits?.extraNormalMaxLevel >= def.level);
  ...
  if (action.type === 'set-support') {
    if (!MAIN.includes(state.phase) || !p.hand.includes(action.card) || !['spell', 'trap'].includes(def?.kind)) return 'invalid-set';
    return Number.isInteger(action.slot) && action.slot >= 0 && action.slot < 5 && !p.support[action.slot] ? null : 'support-zone-required';
  }
  if (action.type === 'position') {
    if (state.phase==='end'&&p.units.includes(action.card)&&def.traits?.endPosition)return card.used.endPosition===state.turn?'position-used':null;
    if (!MAIN.includes(state.phase) || !p.units.includes(action.card)) return 'main-phase-only';
    return card.summonedTurn === state.turn || card.positionTurn === state.turn || card.attacksUsed ? 'position-used' : null;
  }
  if (action.type === 'activate') {
    if (!MAIN.includes(state.phase) && !(def?.subtype==='quick'&&state.phase==='battle')) return 'main-phase-only';
    const fromHand = p.hand.includes(action.card), fromSupport = p.support.includes(action.card);
    if (!fromHand && !fromSupport && !p.units.includes(action.card)) return 'invalid-source';
    if (def?.kind === 'unit' && fromHand && def.traits?.activateFrom !== 'hand' && !def.traits?.ritualEnabler) return 'unit-must-be-on-field';
    if(p.flags.lockedName?.until>=state.turn&&p.flags.lockedName.value===def.name.tr)return 'name-locked';
    if (suppressed(state,action.card)) return 'effect-negated';
    if (def?.kind === 'trap' && (fromHand || card.setTurn >= state.turn)) return 'trap-must-wait';
    if (fromSupport && def?.subtype === 'quick' && card.setTurn >= state.turn) return 'quick-must-wait';
    if (!(def?.effects?.length) && !(def?.kind !== 'unit' && ['continuous','equip','field'].includes(def?.subtype))) return 'no-activated-effect';
  ...
  if (action.type === 'attack') {
    ...
    if (!p.units.includes(action.card) || card.face !== 'up' || card.position !== 'attack' || card.attacksUsed >= (trait(state, action.card, 'attacks') || 1)) return 'attack-used';
    if (attackBlocked(state,action.player,action.card,action.target)) return 'attack-blocked';
    return attackTargets(state,action.player,action.card).includes(action.target||null)?null:'invalid-battle-target';
  }
...
export function responseCards(state) {
  ...
  const who = state.pending.responding, p = state.players[who];
  return [...p.support,...p.hand,...p.units,...p.grave].filter(Boolean).filter(uid => {
    const def=definition(state,uid);
    return def.response?.includes(state.pending.action.type)&&canRespond(state,uid,who);
  }).sort((a,b)=>Number(definition(state,b).subtype==='counter')-Number(definition(state,a).subtype==='counter'));
}

function resolve(state, action) {
  const ctx = context(state, action.player, action.card, action.targets), p = state.players[action.player];
  const card = state.cards[action.card], def = definition(state, action.card);
  if(action.type==='draw') {
    if(flagActive(state,action.player,'skipDraw'))delete p.flags.skipDraw;else ctx.draw(action.player);
    state.phase='standby';ctx.emit('standby',{player:action.player});
  }
...
export function dispatch(original, action) {
  ...
  if (action.type === 'surrender') finish(state, 1 - action.player, 'surrender');
    ...
    if(state.choice.kind==='option')job.effects.unshift(...state.choice.options.find(o=>o.id===action.option).effects);
    ...
    if(state.phase==='draw'||state.phase==='main1'&&state.turn>1) {
      const declared={type:state.phase==='draw'?'draw':'battle-start',player:state.active,revision:state.revision};
      state.pending={action:declared,responding:1-state.active,negated:false};
      if(!responseCards(state).length){resolve(state,declared);state.pending=null;}
      settle(state);checkPoints(state);
      canonicalize(state);
    ...
    if (index >= PHASES.length) {
      for(const uid of Object.keys(state.cards)) {
        const c=state.cards[uid],at=locate(state,uid);
        if(!at)continue;
        if(c.returnControl?.turn===state.turn) {
          const owner=c.returnControl.player,slot=state.players[owner].units.indexOf(null);delete c.returnControl;
          if(slot>=0)ctx.move(uid,'units','control-return',owner,slot);else ctx.move(uid,'grave','control-return-no-zone');
        }
        if(modified(state,uid,'endGrave'))ctx.move(uid,'grave','end-effect');
      }
      state.turn++; state.active = 1 - state.active; index = 0;
      for (const player of state.players) { player.normalUsed = 0; player.flags = Object.fromEntries(Object.entries(player.flags).filter(([key])=>key==='skipDraw')); }
      for (const c of Object.values(state.cards)) { c.attacksUsed = 0; c.modifiers = c.modifiers.filter(m => m.until === null); }
    }
    ...
    ctx.move(action.card, 'support', 'set', action.player, action.slot); card.face = 'down'; card.setTurn = state.turn;
    ...
    const flip = card.face === 'down'; card.face = 'up'; card.knownTo = [true, true];
    card.position = flip || card.position === 'defense' ? 'attack' : 'defense'; card.positionTurn = state.turn;
    ...
    if (action.type === 'activate') { card.used.activate = state.turn; card.used.duelActivated = true; }
    state.pending = { action, responding: 1 - action.player, negated: false };
    if (!responseCards(state).length) { resolve(state, action); state.pending = null; }
  }
  settle(state);
  checkPoints(state);
  canonicalize(state);
