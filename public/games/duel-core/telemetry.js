...
export function recordAction(tel, prev, next, action) {
  ...
  const destroys = newLog.filter((e) => e.event === "destroy" || (e.event === "move" && e.to === "grave")).length;
  ...
  const id = def?.id;
  if (id && ["summon", "set-unit", "set-support", "activate", "attack", "special", "respond"].includes(action?.type)) {
  ...
  tel.events.push({
    ...
    card: id || null,
    name: def?.name || null,
    target: action?.target ? cardId(next, action.target) : action?.target === null ? "direct" : null,
    delta,
    destroys,
    draws,
    summons,
