...
The live runtime uses version 2 under `tariklab.nextwave.son-100-gun.slot<1|2|3>`. Version 1 saves migrate in place. History and the Wave 2 phase, preparation, actor-memory, chain and resolved-crisis ledgers are bounded. See `WAVE2_IMPLEMENTATION.md` for current implementation truth; this document otherwise remains a historical design contract.
