...
test("safe local sessions isolate slots, preserve active state on failed load, recover backup and report quota failures", () => {
  ...
  try {
    ...
    session.beginNew();
    session.commitNew();
    session.state.playerName = "Bir";
    session.save(1);
    session.save(2);
    session.state.playerName = "İki";
    session.save(2);
    session.load(1);
    ...
    values.set("tariklab.nextwave.hayat.slot2", "{broken");
    session.load(2);
    assert.equal(session.state.playerName, "Bir");
    fail = true;
    ...
    fail = false;
    session.remove(2);
    assert.equal(values.has("tariklab.nextwave.hayat.slot2.backup"), false);
    session.load(1);
    assert.equal(session.state.playerName, "Bir");
